...
class SchemaTest(unittest.TestCase):
    ...
    def test_browse_artists(self):
        ...
        with self.connection as c:
            self.assertEqual(map(ref, self.artists), schema.browse(c, Ref.ARTIST))
            self.assertEqual(
                map(ref, self.artists),
                schema.browse(c, Ref.ARTIST, role=["artist", "albumartist"]),
            )
            self.assertEqual(
                map(ref, self.artists[0:1]), schema.browse(c, Ref.ARTIST, role="artist")
            )
            self.assertEqual(
                map(ref, self.artists[0:1]),
            ...
            self.assertEqual(
                map(ref, self.artists), schema.browse(c, Ref.ARTIST, role="albumartist")
            )
    ...
    def test_browse_albums(self):
        ...
        with self.connection as c:
            self.assertEqual(map(ref, self.albums), schema.browse(c, Ref.ALBUM))
            self.assertEqual(
                map(ref, []), schema.browse(c, Ref.ALBUM, artist=self.artists[0].uri)
            )
            self.assertEqual(
    ...
    def test_browse_tracks(self):
        ...
        with self.connection as c:
            self.assertEqual(map(ref, self.tracks), schema.browse(c, Ref.TRACK))
            self.assertEqual(
